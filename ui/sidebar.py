...
class ConversationSidebar(QFrame):
    ...
    def _init_ui(self):
        ...
        hdr = QHBoxLayout()
        lbl = QLabel('💬 对话')
        lbl.setStyleSheet(f'font-weight:bold; color:{t["title_color"]}; font-size:12px;')
        hdr.addWidget(lbl)
        hdr.addStretch()
        self._btn_new = QPushButton('＋')
        self._btn_new.setFixedSize(24, 24)
        self._btn_new.setToolTip('新建对话')
        self._btn_new.setStyleSheet(f'QPushButton{{border:1px solid {t["input_border"]};border-radius:4px;background:{t["frame_bg"]};color:{t["text_color"]};font-weight:bold;font-size:14px;}} QPushButton:hover{{background:{t["list_item_hover"]};}}')
        ...
        self._list.setStyleSheet(f"""
            QListWidget{{border:1px solid {t["frame_border"]};border-radius:4px;background:{t["list_bg"]};color:{t["text_color"]};font-size:11px;}}
