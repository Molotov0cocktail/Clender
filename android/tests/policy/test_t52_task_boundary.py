"""T52 adds exactly one task document exception; no real files are inspected."""
...
class T52TaskBoundaryTests(unittest.TestCase):
    ...
    def test_exact_t52_task_document_is_allowed(self):
        self.check_paths("doc/tasks/T52-android-scheduling-calendar.md")

    def test_other_t52_names_and_suffixes_remain_forbidden(self):
        for path in (
            "doc/tasks/T52-other.md",
            "doc/tasks/T52-android-scheduling-calendar.md.bak",
            "doc/tasks/T52-android-scheduling-calendar.md/child.md",
            "doc/tasks/T52-android-scheduling-calendar-extra.md",
            "doc/tasks/t52-android-local-time-bugfix.md",
        ):
    ...
    def test_authorized_document_does_not_hide_windows_or_data_changes(self):
        for path in ("database.py", "config.py", "data/config.json", "dist/data/clender.db"):
            with self.subTest(path=path), self.assertRaises(AssertionError):
                self.check_paths("doc/tasks/T52-android-scheduling-calendar.md", path)
